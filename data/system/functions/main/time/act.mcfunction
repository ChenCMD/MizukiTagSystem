...
execute as @e[tag=SM,limit=1] if entity @s[scores={GameTimeA=20..}] if entity @s[scores={GameTimeB=1..}] if entity @s[tag=!HideMode] as @a[team=Escape] unless entity @s[scores={InvisibleArmor=0..}] run function system:armor/escape
execute as @e[tag=SM,limit=1] if entity @s[scores={GameTimeA=20..}] if entity @s[scores={GameTimeB=1..}] run function system:main/time/check_100sec
