...
execute as @e[tag=SM,limit=1] if entity @s[scores={GameTimeA=20..,GameTimeB=1..,LoDDeley=0}] if entity @e[tag=SM,tag=!HalloweenEvent] run tellraw @a {"text":"Luck of Diceが使用可能になった！","color":"green"}
execute as @e[tag=SM,limit=1] if entity @s[scores={GameTimeA=20..,GameTimeB=1..,LoDDeley=0}] if entity @e[tag=SM,tag=HalloweenEvent] run tellraw @a {"text":"お菓子箱を開けれるようになった！","color":"green"}
