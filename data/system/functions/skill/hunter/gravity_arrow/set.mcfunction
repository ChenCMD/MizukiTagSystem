...
data merge entity @e[type=arrow,tag=GravityArrow,distance=..5,sort=nearest,limit=1] {Color:-1,crit:0b,life:1s}
...
execute if entity @e[type=arrow,tag=GravityArrow,distance=..5,sort=nearest,limit=1] run give @s arrow{display:{Name:"\"§5魔力\""}}
kill @e[type=arrow,tag=GravityArrow,distance=..5,sort=nearest,limit=1]