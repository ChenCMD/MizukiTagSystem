...
execute if entity @e[type=arrow,distance=..5,sort=nearest,limit=1] unless entity @s[team=Hunter,level=0,nbt=!{ActiveEffects:[{Id:27b}]},tag=GravityArrowReady] unless entity @s[team=Hunter,level=0,nbt=!{ActiveEffects:[{Id:27b}]},tag=ChaseArrowReady] run data merge entity @e[type=arrow,distance=..5,sort=nearest,limit=1] {Color:-1,crit:0b}
