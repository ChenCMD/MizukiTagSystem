...
execute if entity @s[tag=SnowCheckPassed] run scoreboard players operation $Reg SnowScore %= $3 Ver
execute if entity @s[tag=SnowCheckPassed] run execute if score $Reg SnowScore matches 0 run playsound block.snow.break block @a[distance=..9] ~ ~ ~ 0.5
execute if entity @s[tag=SnowCheckPassed] run execute if score $Reg SnowScore matches 0 run particle block snow ~ ~ ~ 0.2 0 0.2 1 5 normal @a[distance=..9]
